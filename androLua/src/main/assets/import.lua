...
local new = org.keplerproject.luajava.new
...
local function import_class (classname,packagename)
    local res,class = pcall(org.keplerproject.luajava.bindClass,packagename)
    if res then
        _G[classname] = class
...
function proxy (classname,obj)
	...
	if classname:find('.',1,true) then
		return org.keplerproject.luajava.createProxy(classname,obj)
	end
	-- otherwise, it must lie on the package path!
	for i,p in ipairs(packages) do
		local ok,res = pcall(org.keplerproject.luajava.createProxy,p..classname, obj)
